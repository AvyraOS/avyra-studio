...
type BrandGuideProps = {
  ...
  pdfUrl?: string // For future PDF support
}
...
type ProjectSectionProps = {
  ...
  pills?: string[] // For overview variant
  imageSrc?: string
  imageAlt?: string
  imageSide?: 'right' | 'left'
  scopeLabel?: string // For overview variant
  className?: string
  id?: string
  centerImage?: boolean
...
const ProjectSection = forwardRef<HTMLElement, ProjectSectionProps>(({
  ...
  // Brand Guide Variant - Full width image section
  if (variant === 'brand-guide') {
    const guideImageSrc = brandGuide?.imageSrc || imageSrc
    const guideImageAlt = brandGuide?.imageAlt || imageAlt || 'Brand Guide'
    
    return (
      <section ref={ref} id={id} className={clsx("panel relative w-full min-h-screen overflow-hidden bg-[#0d0d0d]", className)}>
        {guideImageSrc && (
          <Image
            src={guideImageSrc}
            alt={guideImageAlt}
  ...
  // Overview and Testimonial Variants - Split layout with image and content
  const showScope = variant === 'overview' && pills.length > 0
  const showTestimonial = variant === 'testimonial' && testimonial
  ...
  return (
    <section ref={ref} id={id} className={clsx("panel relative w-full min-h-0 sm:min-h-[600px] md:min-h-screen overflow-hidden bg-[#0d0d0d]", className)}>
      ...
      <div className={clsx(
        ...
        imageSrc && (isRight ? "lg:grid-cols-[1fr_1.1fr]" : "lg:grid-cols-[1.1fr_1fr]")
      )}>
        {/* Text column */}
        <div className={clsx("z-[1] md:max-w-[600px] lg:max-w-none", isRight ? "order-1" : "order-2 lg:pl-8")}> 
          ...
          {descriptionParagraphs.length > 0 && (
            <div className={clsx("w-full max-w-full sm:max-w-[52ch] md:max-w-[56ch] lg:max-w-[58ch] text-[#d5dbe6] text-[14px] sm:text-[15px] md:text-[15.5px] lg:text-[16px] leading-[22px] sm:leading-[24px] md:leading-[25px] lg:leading-[25.6px] tracking-[-0.28px] sm:tracking-[-0.30px] md:tracking-[-0.31px] lg:tracking-[-0.32px] font-inter font-normal", title ? "mt-4 sm:mt-5 md:mt-6" : "")}>
              {descriptionParagraphs.map((text, idx) => (
          ...
          {/* Scope Section - Only for overview variant */}
          {showScope && (
            <>
          ...
          {/* Testimonial Section - Only for testimonial variant */}
          {showTestimonial && (
            <div className={clsx("mt-12 sm:mt-14 md:mt-20 lg:mt-[100px]")}>
              {/* Rating Stars */}
              <div className="flex gap-0.5 mb-4">
